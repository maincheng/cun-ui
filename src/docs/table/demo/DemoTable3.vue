<script lang="ts" setup>
    ...
    const set = (row: any, column: any) => {
        CunMessage.info(`你可以拿到这一行的数据，例如：id=${row.id}`);
        console.log(row, column);
    };
    const del = (row: any, column: any) => {
        CunMessage.warning(`你也可以拿到这一列的数据，例如：field=${column.field}`);
    };
</script>
...
<template>
    <cun-table :data-source="data" :columns="columns">
        ...
        <template #action="{ row, column }">
            <cun-flex gap="5">
                <cun-button text="管理" size="small" type="text" @click="set(row, column)" />
                <cun-button text="删除" size="small" type="text" @click="del(row, column)" />
            </cun-flex>
        </template>
    </cun-table>
</template>
