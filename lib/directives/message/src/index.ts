...
const createMessageList = () => {
    ...
    messageContainer.setAttribute('id', 'cun-message');
    document.body.appendChild(messageContainer);
};

const showMessage = ({ type, e }: MessageOptions) => {
    const messageContainer: any = document.getElementById('cun-message');
    ...
    if (!hasMessageById) {
        ...
        clearTimeout(CunMessage.timer[e.id]);
    }
    ...
    setTimeout(() => {
        ...
        CunMessage.timer[e.id] = setTimeout(() => {
...
const CunMessage: any = {
    name: 'CunMessage',

    message: ({ type, e }: MessageOptions) => {
        if (!document.getElementById('cun-message')) {
            createMessageList();
            CunMessage.message({ type, e });
        } else {
            showMessage({ type, e });
        }
    },
    ...
    close: ({ id }: { id: string }) => {
        ...
        if (messageElement) {
            ...
            setTimeout(() => {
                clearTimeout(CunMessage.timer[id]);
            ...
            asyncFn()
                .then(
                    ({
                        ...
                        // 隐藏loading消息
                        CunMessage.close({ id: 'request-loading' });
...
CunMessage.warning = (e: MessageFnOptions) => CunMessage.message({ type: 'warning', e });
CunMessage.error = (e: MessageFnOptions) => CunMessage.message({ type: 'error', e });
CunMessage.info = (e: MessageFnOptions) => CunMessage.message({ type: 'info', e });
CunMessage.normal = (e: MessageFnOptions) => CunMessage.message({ type: 'normal', e });
CunMessage.success = (e: MessageFnOptions) => CunMessage.message({ type: 'success', e });
CunMessage.loading = (e: MessageFnOptions) => CunMessage.message({ type: 'loading', e });

export default CunMessage;
