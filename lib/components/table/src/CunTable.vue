<script setup lang="ts">
    ...
    import { CunFlex, CunCheckbox, CunTextTrim } from 'cun-ui';
...
<template>
    <div
        class="cun-table-wrapper"
        :class="{
            'cun-table-checkable': props.checkable
        }"
    >
        <div
            ...
            class="cun-table-scroll-line-left"
        ></div>
        <div
            ...
            class="cun-table-scroll-line-right"
        ></div>
        <div
            ref="tableRef"
            class="cun-table cun-scrollbar"
            :style="`max-height: ${any2px(maxHeight)}`"
            @scroll="checkScroll"
            @mouseleave="state.hoverIndex = -1"
        >
            <div
                class="cun-table-head"
            ...
            >
                <div
                    ...
                    ref="fixedLeftRef"
                    class="cun-table-fixed-left"
                >
                    <div class="cun-table-tr">
                        <cun-flex
            ...
            <div
                ...
                class="cun-table-body"
                :style="`width: ${getTdTotalWidth}px`"
            >
                <div v-if="fixedColumns('left').length > 0" class="cun-table-fixed-left">
                    <div
                        v-for="(row, i) in dataSource"
                        :key="`data${i}`"
                        class="cun-table-tr"
                        :class="{
                            'cun-table-tr-hover': state.hoverIndex === i,
                            'cun-table-tr-selected': checkable && state.selectedKeysMap[row[rowKey]]
                        }"
                        @click="selectTr(row)"
                    ...
                    >
                        <cun-flex
                            ...
                            style="width: 50px"
                            x="center"
                            class="cun-table-checkbox-wrapper"
                        >
                            <cun-checkbox
                                class="cun-table-checkbox"
                ...
                <div class="cun-table-main">
                    <div
                        ...
                        :class="{
                            'cun-table-tr-hover': state.hoverIndex === i,
                            'cun-table-tr-selected': state.selectedKeysMap[row[rowKey]]
                        }"
                        @click="selectTr(row)"
                        @mouseenter="state.hoverIndex = i"
                    >
                ...
                <div v-if="fixedColumns('right').length > 0" class="cun-table-fixed-right">
                    <div
                        v-for="(row, i) in dataSource"
                        :key="`data${i}`"
                        class="cun-table-tr"
        ...
        <cun-flex v-if="dataSource.length === 0" style="padding: 50px 0px">
            <cun-empty />
        </cun-flex>
    </div>
</template>
...
<style lang="scss">
    ...
    .cun-table {
        ...
        .cun-table-tr {
            display: flex;
            background-color: var(--cun-bgcolor-0);
            border-bottom: var(--cun-table-border);
            overflow: hidden;
            width: 100%;
            box-sizing: border-box;
            flex-grow: 1;
        ...
        .cun-table-head {
            ...
            .cun-table-tr {
                background-color: var(--cun-table-header-bgColor);
                height: 40px;
                flex-shrink: 0;
                border-bottom: var(--cun-table-border);
        ...
        .cun-table-checkbox-wrapper {
            position: relative;
            cursor: pointer;
        }

        .cun-table-checkbox-wrapper::after {
            position: absolute;
            z-index: 1;
            content: '';
            top: 0px;
        ...
        .cun-table-tr-selected {
            background-color: var(--cun-table-tr-selected);
        }
    }
</style>